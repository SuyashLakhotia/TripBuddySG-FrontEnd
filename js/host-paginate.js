$(document).ready(function () {
    ...
    var number_of_items = $('#hosts-grid').find('.host-grid-link').size(); // total number of items
    ...
    $('#host_page_nav').html(navigation_html);

    // Add "active" class to first page link:
    $('#host_page_nav li:first').addClass('active');

    
    // Hide all elements inside grid:
    $('#hosts-grid').children().css('display', 'none');

    // Show the first n (show_per_page) elements:
    $('#hosts-grid').children().slice(0, show_per_page).css('display', 'block');
});

function scroll_to_start() {
    $(document).scrollTop($("#hosts-grid").offset().top);
}
...
function go_to_page(page_num) {
    ...
    // Hide all the children elements of .hosts-grid and then show the specific slice:
    $('#hosts-grid').children().css('display', 'none').slice(start_from, end_on).css('display', 'block');
